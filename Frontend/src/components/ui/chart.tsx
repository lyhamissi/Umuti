...
export type ChartTooltipContentProps = {
  ...
  labelKey?: string;
  label?: string;
  labelFormatter?: (label: any, payload?: CustomPayloadItem[]) => React.ReactNode;
    ...
    return (
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item: CustomPayloadItem, index: number) => {
    ...
    return (
      <div ref={ref} className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
        {payload.map((item: LegendItem, index: number) => {
          ...
          const itemConfig = getPayloadConfigFromPayload(config, item, key);
          return (
            <div key={item.value || key} className={cn("flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground")}>
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: any, key: string) {
  ...
  const configLabelKey = key in payload ? key : payloadPayload && key in payloadPayload ? key : key;
  return config[configLabelKey as keyof ChartConfig];
}

// -------------------- Exports --------------------
export { ChartContainer, ChartTooltipContent, ChartLegendContent, ChartStyle, Tooltip as ChartTooltip, Legend as ChartLegend };
